...
class NumericOperations {
    public constructor(f: any) {
        ...
        f.defjs("s>d", function singleToDouble() {
            var value = fromInt(f.stack.pop());
            f.stack.push(value.low);
            f.stack.push(value.high);
        });

        f.defjs("*/", function multiplyDivide() {
            var divisor = fromInt(f.stack.pop());
            var first = fromInt(f.stack.pop());
            var second = fromInt(f.stack.pop());
        ...
        f.defjs("um*", function () {
            var first = fromInt(f.stack.pop(), true);
            var second = fromInt(f.stack.pop(), true);
            var result = first.mul(second);
            f.stack.push(result.low);
            f.stack.push(result.high);
        });
